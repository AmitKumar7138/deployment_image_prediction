...
## Introduction
In this lab we will go through the process of deploying an already trained Deep Learning model. To do so, we will take advantage of the user-friendly library fastAPI that provides a nice REST API framework.
 
This lab is specifically designed to run locally on our machine. This can be done via 2 methods:  using `Python Virtual Environments` or using `Docker`. 
...
As a general note, the commands in this tutorial are meant to be run within a terminal. To begin you need to **clone this repo in your local filesystem and `cd` to the week1--lab directory**.
...
```bash
cd machine-learning-engineering-for-production-public/course1/week1--lab
```
This will bring you to the `week1--lab` directory. The `ls` command allows you to list the files and directories.
Type `ls` and let's take a quick look at the content inside `week1--lab` directory:
 
```
.
└── week1--lab (this directory)
    ├── images (includes some images from ImageNet)
    ├── server.ipynb (Part 1 of the  lab)
    ├── client.ipynb (Part 2 of the  lab)
...
You will use Conda as an environment management system so that all the dependencies you need for this lab are stored in an isolated environment.
...
At this point, you will do all your libraries installation and work in this environment. So, whenever working on this  lab, check the mlep-w1-lab environment is active.
...
Before proceeding, double check that you are currently on the `week1--lab` directory, which includes the `requirements.txt` file. This file lists all the required dependencies and their respective versions. 
...
Using the following command will download or pull the image necessary to run this  lab locally:
```bash
...
Images can also be thought of as the blueprints for containers, which are the actual instances of the software running. To run a container using the image you just pulled, double check that you are currently on the `week1--lab` directory and use this command:
```bash
...
- -p: Allows you to map a port in your computer to a port in the container. In this case we need a port for the Jupyter server and another for the server you will run within the  lab.
- --mount: Allows you to mount a directory in your local filesystem within the container. This is very important because if no mounts are present, changes to files will not persist after the container is deleted. In this case we are mounting the current directory `week1--lab` onto the `/home/jovyan/work` directory inside the container.
...
Once you have authenticated, click in the `/work` directory and you should see all of the files from your current local directory. Look for the `server.ipynb` file and open it to begin the  lab.
